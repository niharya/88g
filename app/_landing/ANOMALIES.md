...
- **Startooth engine vocabulary** — voids/keys/faces/top terminology and the static-data-lives-in-constants rule.
...
- **Expanded state clips the overspilled plate on the X axis** — `.landing--expanded` `overflow-x: clip` swallows the phantom scrollbar from the 1.02-scaled viewport-width plate.
...
**What it is.** In `:root` (`landing.css`), only `--hero-top` + `--hero-h` are free measurements. `--projects-top`, `--long-top`, `--spectrum-top`, `--practice-top`, `--contact-top`, and `--expanded-h` are DERIVED as a `calc()` chain off the settled hero bottom (`--hero-top + --hero-h`): each value = the block above's position + that block's height + its gap (e.g. `--spectrum-top: calc(var(--long-top) + <about-long block>)`, where the inline comment names the block's real height + gap). `--projects-top` and `--long-top` tuck a few px UP under the hero (the `- 6px` / `- 31px` terms) so the markers + about-long's statement read as docked into the card. **The cascade gap cadence is ~54px** (about-long→spectrum, spectrum→practice, practice→contact all target it) — NOT the `--stack-gap` 40px design token; that token is the Group-B *entrance* rhythm, the 54px is the settled dock cadence. Don't conflate them.
...
**Hover hatch reconciled to our motion; segments are not controls.** The per-segment hatch (dots/rules/crosshatch) rides a `::after` overlay that fades between its rest and hover/focus alpha via a `transition` (`--ease-paper`) — NOT the handoff's instant `background-image` swap — so feedback eases like the rest of the site. Segments are tactile surfaces only: `cursor` stays `auto` (no false click affordance, per the "controls must not lie" hard rule). The REC dot pulse is the `recpulse` keyframe (1.4s), guarded by `prefers-reduced-motion: reduce`. Don't restore the instant swap, and don't add a click cursor/handler to segments.

**Desktop rest-state "years" unit — Product (05) only.** At rest, only the PRODUCT segment shows its "years" unit beside the numeral (`.practice-timeline__seg--product:not(.is-collapsed) .practice-timeline__seg-unit`, in the desktop `@media (min-width:768px) and (min-height:501px)` gate) — the longest phase anchors the axis, so its unit stays legible while Interface (02) + Brand (03) stay BARE until focused. Mobile is unchanged: the base rule keeps all three units focus-gated (this desktop gate is its exact complement). Don't show the unit on Interface/Brand at rest — the narrow columns can't hold it.

**Hover affordance — "leans open", per-segment technique split (load-bearing).** On hover (desktop-gated: `@media (hover:hover) and (min-width:768px) and (min-height:501px)`) a non-focused (`.is-default`) segment strengthens its hatch (`::after` opacity rest → hover) and leans OPEN toward where it grows on focus. The technique DIFFERS by column width, deliberately:
- **Product (wide):** a standalone `scale: 1.04 1` with `transform-origin: right center` — because the column is wide, the scaleX carries its CONTENT leftward over the neighbour, reading as a real push in the direction it opens. Standalone `scale:` (snap tier) composes with any transform and doesn't reflow.
- **Interface + Brand (narrow):** they GENUINELY widen via `width: calc(var(--phase-size) + 12px)` — the SAME lever focus uses — nudging the neighbours over. A scaleX on a narrow column would just stretch its text; widening grows it properly. A SINGLE animated property (width), so nothing composes and there's no hover-out jump.

REJECTED for the hover-open: a `translateY` lift (read as a button PRESS, not "opens"); and for Interface/Brand a counter-scale technique (box `scaleX` + inverse content `scale` to keep text unstretched) — the two nested transforms drift and JUMP on hover-out. Don't reach for either.
...
The terms the engine (`StartoothField`) and its docs use for the lattice elements:
...
- **Static data** — all geometry, palette, and timing live in `startooth-constants.ts`. `StartoothField` holds them as `private readonly` aliases; method bodies are unchanged. Edit a constant in `startooth-constants.ts` only.

## Framed sheet — the canvas is a centered page, not full-bleed (DESKTOP-ONLY for now)

**What it is.** The Startooth canvas is no longer full-bleed. It's clipped into a centered "page" with a brown frame, sitting on the `/all` desk ground. On desktop/iPad the sheet **fills the viewport minus a uniform `--sheet-margin` on all sides — no fixed ratio, no width cap** (the old 3:4/4:3 orientation-dependent ratio and the 760px cap were DROPPED; the landscape override was removed entirely). Two cooperating fixed elements build it (both in `startooth-canvas.css`, rendered by `StartoothCanvas.tsx`):
...
**Sheet geometry — VIEWPORT-FILL on desktop/iPad, on `:root` in `startooth-canvas.css`.** Don't restate the numbers the tokens own; the structure is:

- `--sheet-margin` — the UNIFORM inset of the sheet from the viewport edge, all four sides (top/bottom via the root+matte `top`/`bottom`; left/right via the width formula below).
- `--sheet-width` on desktop/iPad (the `@media (min-width:768px) and (min-height:501px)` `:root` override) = `calc(100vw − 2 * var(--sheet-margin))`. The sheet GROWS to fill the viewport width minus the same margin as top/bottom — no ratio, no cap. Height is already viewport-driven (root+matte `top`/`bottom: var(--sheet-margin)`), so the sheet tracks the viewport's OWN proportions with a uniform margin all round. The fixed-px hero column just centres on a broader/taller pattern field on large monitors — it never shrinks. The `min-height:501` guard keeps landscape phones (>768 wide, <501 tall) out of the framed sheet (they stay full-bleed mobile).
- The BASE `--sheet-width` `min(calc((100svh − 2*--sheet-margin)/1.3333), 760px, calc(100vw − 2*var(--sheet-gutter)))` (the old 3:4-portrait / 760-cap formula) still EXISTS but is now only the pre-gate first-paint + mobile fallback — INERT on any real desktop/iPad viewport (the override above wins) and never actually read on mobile (the reset block sets `width: auto`). Don't reason about the sheet's live desktop size from this base — read the gate.
- `--sheet-shift` keeps a `16px` BASE (mobile = live), but is **zeroed to `0` on iPad+desktop** (same `@media` `:root` override) so the sheet sits dead-centre on the viewport, matching the centred content + footer — see "Sheet harmonization (Phase 3)" → "Viewport-centred". (It's a literal, not `var(--gutter)`, because `.startooth-canvas-root`/`.startooth-sheet-matte` sit OUTSIDE `.landing` and can't read its scoped token.)
...
The framed sheet (above) now FILLS the viewport (minus a uniform `--sheet-margin`); the landing CONTENT (`.landing__section--*`) is authored at FIXED px. Because the sheet is viewport-sized, the fixed-px column always fits inside it — the old fit-to-frame clamp (which existed to shrink the plate when the frame shrank below the content's 760 baseline) is no longer needed and was REMOVED. What remains is a light **readability nudge**: a flat `1.02` content zoom on desktop/iPad (`1` on mobile), NOT a frame-coupled scalar. **All of it is gated to `@media (min-width: 768px) and (min-height: 501px)`** — the exact COMPLEMENT of the mobile block — so mobile keeps its full-bleed composition (enforced by the reset block — see "Framed sheet") and the base rules carry no Phase-3 changes. This sheet is a *page* member of the shared family (LIBRARY.md → "Framed-sheet spine").

**Scale nudge — a flat readability zoom, decoupled from frame width.** `page.tsx` writes `--sheet-scale` on `:root` as a flat literal: `1.02` on desktop/iPad, `1` on mobile — a tiny `matchMedia('(max-width:767px),(max-height:500px)')` listener setting one or the other, no measurement. It replaced the old `min(1, (frameWidth − 2*inset)/760)` clamp: the resize/`ResizeObserver` measure machinery, the `--sheet-content-inset` read, and the 760 baseline are all GONE — the sheet fills the viewport so content always fits, and 1.02 is a gentle up-nudge for legibility, not a fit-to-frame shrink. `landing.css` still applies `transform: scale(var(--sheet-scale, 1))` + `transform-origin: 50% top` to `.landing__content`, still scales the EXPANDED scroll height (`.landing--expanded` and the form-open `:has()` bump) by the same factor so the plate's true height matches its visual height, and the footer slab + nav counter-scale still read `--sheet-scale` (so it's the single knob). Default `1` (SSR / no-JS / pre-mount) = design size. **Containing-block safety (load-bearing):** the transform makes `.landing__content` a containing block for any `position: fixed` descendant — but the canvas root + matte (siblings of `.landing`), `.caption-tag` (inside `.landing`, OUTSIDE `.landing__content`), and the footer (outside `.landing` entirely) are all OUTSIDE the wrapper, so the frame is never trapped or scaled. Don't move a fixed edge element inside `.landing__content`. The sections' own Group A/B `translateY` settles compose UNDER the scale (motion intact); `--sheet-scale` must live on `:root`, not `.landing`, so the sibling footer can read it. **Because the plate is now viewport-width, a `--sheet-scale > 1` spills the plate a few px past the viewport — see "Expanded state clips the overspilled plate on the X axis" for the guard that swallows the phantom scrollbar.**
...
**No width cap on the sheet — the fixed hero column just centres on a broader field.** The sheet fills the viewport (see "Framed sheet" → geometry), so there is no ratio and no 760px cap. On a large monitor the hero column doesn't grow with the sheet; it sits centred on a broader/taller pattern field. (The blue `/all` card still keeps its own relative cap because it's an aspect-locked card that nests concentrically on iPads — a different height model; don't copy a cap back onto this sheet.)

**What breaks if violated.** Remove the `@media` gate → the scale/centring leak onto mobile (which must stay full-bleed). Set `--sheet-scale` on `.landing` instead of `:root` → the sibling footer can't read it. Move a fixed edge element inside `.landing__content` → the scale traps it. Forget to scale `.landing--expanded` height → the plate's true height desyncs from its visual height, leaving dead scroll. Restore the footer to `z:20` → it floats over the frame again instead of tucking under it. Reintroduce a frame-width-coupled `--sheet-scale` clamp (measuring the frame) → you re-add the removed resize machinery for no gain, since the sheet now fills the viewport and content always fits.

## Nav row is scale-exempt (counter-scales the plate)

**What it is.** The landing nav row (`.landing-nav-row` in `landing.css` — the Nihar + Works tabs docked under the hero) counters the plate scale ONLY when the plate SHRINKS, so it never reads too small. It rides `.landing__content`'s `transform: scale(var(--sheet-scale))` like everything else, then cancels it locally with a SHRINK-ONLY clamp: `scale: max(1, calc(1 / var(--sheet-scale, 1)))` + `transform-origin: top center`. When `--sheet-scale < 1` the `1/scale` term (>1) wins and holds the nav at constant size; when `--sheet-scale > 1` (the current flat `1.02` readability nudge, and any future up-nudge) the `max(1, …)` caps the counter at `1`, so the nav grows WITH the plate rather than shrinking against it. A no-op at scale 1 (mobile, where `--sheet-scale` is forced to 1). Fixed the tabs reading "too small" on scaled-down frames without fighting the up-nudge.
...
## Expanded state clips the overspilled plate on the X axis

**What it is.** `.landing--expanded` (in `landing.css`) carries `overflow: visible; overflow-x: clip` — the `overflow-x: clip` is the load-bearing addition (the base was just `overflow: visible`).

**Why.** The content plate (`.landing__content`) is now viewport-width, and `--sheet-scale` is a flat `1.02` on desktop/iPad (see "Sheet harmonization (Phase 3)"). Any `--sheet-scale > 1` scales the full-width plate a few px PAST the viewport on each side. The visible centred cards are fine — it's the INVISIBLE full-width plate edges that spill. In the expanded state `.landing` is `overflow: visible` (required: the tall expanded plate must scroll with the document), so that spill raised a PHANTOM horizontal scrollbar for a few px of empty overhang.

**Why `clip`, not `hidden`.** `overflow-x: clip` trims the X overspill WITHOUT establishing a scroll container — so the vertical axis stays a normal document-scrolling box (`overflow: visible`) and the tall expanded plate still scrolls with the page. `overflow-x: hidden` was REJECTED: it risks turning the element into a scroll container (which would trap the vertical scroll and can affect `position: fixed` descendants). Also REJECTED: reverting the `1.02` scale to kill the spill at its source — the readability nudge is wanted.

**What breaks if removed.** Drop `overflow-x: clip` and the expanded landing shows a phantom horizontal scrollbar from the scaled-plate overhang. Change it to `overflow-x: hidden` and you risk making the expanded landing a vertical scroll container (trapped scroll / fixed-positioning side effects).
...
2. `useLayoutEffect` at mount: skips the build if `builtThisLoad` is set OR the visitor arrived via `sessionStorage['nav-direction'] === 'to-landing'` (a client-side RETURN from a works route). If either, it sets `built`, `skipBuild`, `buildStarted`, `staged`, `captionIn`, and `frameIn` to `true` synchronously before first paint, collapsing the build to a single settled frame. The nav-direction trigger covers the case where the visitor left BEFORE the ~7s intro finished — `builtThisLoad` was never set, but the return must still hold the settled pattern, not replay the build.
...
**The `nav-direction` flag is a one-shot token consumed IN THE BUILD GATE (not the slide-in effect).** The build gate's `useLayoutEffect` reads `sessionStorage['nav-direction']` and, if it's `to-landing`, `removeItem`s it in the same pass — so the flag is consumed exactly once. The separate slide-in `useLayoutEffect` (directional entrance) reads the same flag but deliberately does NOT clear it, leaving the build gate as the single consumer. A hard refresh has NO flag (fresh session state carries it only across client-side nav), so it still replays the build — this does NOT reintroduce the "`sessionStorage` skips the build on every reload" footgun that keeps `builtThisLoad` module-level (see "Why `builtThisLoad` is a MODULE-LEVEL variable" below).
...
**The skip path also mounts the field on the NEXT FRAME, not an idle slot (`StartoothCanvas.tsx`).** On skip (`skipBuild`), `StartoothCanvas` schedules `field.mount()` via `requestAnimationFrame` — NOT the `requestIdleCallback({ timeout: 300 })` idle slot the fresh-load path uses. On a return the build already collapses to one settled frame (`skipBuild → reduced`), so the settled pattern must paint the INSTANT the landing reappears; the rIC path's up-to-300ms gap left a blank canvas that read as the pattern "restarting". There's no hydration to yield to on a client-side return, so the idle defer buys nothing here. (The fresh-load and grey-hold paths keep their original idle/`setTimeout` mounts — this is skip-only.) Don't route the skip mount back through `requestIdleCallback` — it reintroduces the blank-gap pop.
