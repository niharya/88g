...
### Bench-exit "+Nihar" marker — arrow points LEFT by SPECIFICITY, not source order (load-bearing)
The `+Nihar` marker (`.bench-exit`, `BenchExitMarker`, icon `arrow_back`) is a back-to-landing link and must point LEFT. But nav.css rotates every exit-marker arrow 180° for the shell/marks exits (`.nav-marker--exit .nav-icon { transform: rotate(180deg) }`), so bench.css resets it.

- **The reset selector MUST be three classes: `.bench-exit .nav-marker--exit .nav-icon { transform: none }`** — it has to out-specify the base (`.nav-marker--exit .nav-icon`, two classes). The previous reset (`.bench-exit .nav-icon`, also two classes) won only by SOURCE ORDER; a client-side nav into /all (landing → works → /all) re-inserts the CSS chunks in a different order, the base rotation resurfaced, and the arrow flipped to point RIGHT. **Don't drop a class from this selector** or the flip returns.
- Same block, minor: the Nihar `.nav-marker__exit-label` is route-scoped in bench.css to a dotted-at-rest → solid-on-hover underline (the site's link idiom), overriding nav.css's shell-exit transparent-rest → dotted-hover treatment; also wins by three-class specificity. Don't "unify" it back to the shell treatment.
