...
- **`useBenchDock`** (mirrors the nav cluster's `useDockedMarker`): `active` (vis|lf, default vis) + `docked`. A scroll listener flips `docked` when the ticket's slot reaches the dock point (`slot.top <= 14`); `.is-docked` lifts the ticket OUT (`position:fixed`, top:14, centred via `left:50%` + a constant `transform:translateX(-50%)`) into a condensed navbar, and the condense (width 460→296 + deeper shadow) is CSS keyed on `.is-docked`. There is NO scale — an earlier `scale(0.96→1)` "rise" read as an odd enlarge mid-condense and was removed; `transform` stays a constant `translateX(-50%)` so only `width` + `box-shadow` animate. The slot's `min-height` is pinned (rest footprint) so the card doesn't collapse when the ticket lifts out. The rest→fixed flip is scroll-coincident → seamless (no jump). Entry unifies: scroll docks · a tab click `scrollGlide`s to the dock point · ✕ `scrollGlide(0)` back to the card (no fixed→static hand-off → no end jerk). Switching tabs while docked keeps scroll position.
- **Compositor-first condense (don't reintroduce the churn).** The condense was re-authored to animate as one calm gesture instead of ~9 simultaneous layout animations. The disappearing chrome animates `opacity`/`transform` only and is taken OUT of flow so it can't reflow the rest: (1) the **eyebrow** is `position:absolute` (its rest space reserved by the frame's top padding) and fades via opacity + a small `translateY` — NOT a `max-height`/`margin` collapse; (2) the **close** is `position:absolute` (anchored to the frame, right edge) and fades via opacity + `translateX` — NOT a `width:0→30` grow. With the close out of grid flow the old **`.bench-tab__spacer` mirror cell is gone**; the tabs grid is `1fr auto 1fr` with a constant symmetric `padding:0 30px` reserving the close's room, so the tab pair stays centred in BOTH states with zero horizontal animation (verified: docked tab centres 659/781 are symmetric about the 720 centre). The only remaining box-model animations are the deliberate ones — the ticket `width` (460→296), the frame `padding` (height envelope), and the tab title `font-size` (27→18) — all on the same `--dur-settle`/`--ease-snap` so they read as a single settle. Don't bring back the spacer, the width-animated close, or the eyebrow max-height collapse.
