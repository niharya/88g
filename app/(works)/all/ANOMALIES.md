...
- **Condense — definite width that interpolates, flex content-hug centred, in-flow close.** The condense animates as one snappy settle on **`--dur-slide`/`--ease-snap`** (was `--dur-settle`): the `.bench-ticket` transitions `width`/`padding`/`box-shadow`, and the frame `padding`/`border-width`, tab title `font-size`, eyebrow `opacity`+`translateY`, close `opacity`, the divider, and the bead all ride the same `--dur-slide` beat so they read as a single gesture.
  ...
  - **The condensed width is `min(290px, calc(460px * var(--card-scale)))` — a definite ~290px**, not `fit-content` and not a percentage; width can only interpolate between two definite lengths, so the transition runs a clean rest→290. `fit-content` was tried and rejected (intrinsic sizes don't animate; the box snapped). The `min()` is an **inversion guard**: if the rest width (`460×scale`) ever dropped below 290 (scale < ~0.63) the condense would otherwise GROW the box; the cap keeps it a shrink.
  - **Why 290 is FIXED and not scaled by `--card-scale`** (and why this SUPERSEDES the old "even gaps by construction, slack→0" tuning): the tab sub-labels are a fixed **11px** that does NOT scale with `--card-scale`, so a scaled-down condensed box clipped them. At a definite 290 the content fits with **comfortable left/right margins by intent** — the edges are deliberately wider than the inter-item gaps. It is **no longer** the old even-gap, single-token, centred-slack→0 construction; don't try to re-tune it back to even gaps.
  ...
  - **The close is IN-FLOW when condensed (`position:static`, a real flex item), absolute + `opacity:0` at rest.** This is deliberate — it has to occupy a flex cell in the condensed row. At rest it's `position:absolute` (right edge) with `opacity:0`/`pointer-events:none` so it reserves no rest-state space.
