...
- **`useBenchDock`** (mirrors the nav cluster's `useDockedMarker`): `active` (vis|lf, default vis) + **two coupled states, `pinned` + `condensed`** — split so the visual condense can grow back earlier than the position un-pins, without a jump. `Ticket.tsx` concatenates `.is-pinned` (position) and `.is-condensed` (the visual condense) **independently**. `pinned` toggles at the TOP edge (`slot.top <= DOCK_TOP+1`, `DOCK_TOP=14` mirroring `.is-pinned .bench-ticket { top }`), scroll-coincident → `position:fixed; top:14`, centred via `left:50%` + a constant `transform:translateX(-50%)`. `condensed` is **direction-gated**: it shrinks on the top edge going DOWN (the good snap) but GROWS back on the BOTTOM edge going UP (`r.bottom >= DOCK_TOP`, ~one footprint earlier) while still pinned, then the pin hands off at the top edge — and it is NEVER condensed while unpinned (`if (!nextPinned) condensed = false`). Direction-gating keeps the two edges from fighting in the overlap zone. There is NO scale — an earlier `scale(0.96→1)` "rise" read as an odd enlarge mid-condense and was removed; `transform` stays a constant `translateX(-50%)` so only `width`/`padding`/`box-shadow` animate. The hook also pins the slot's `min-height` (rest footprint, `useLayoutEffect`) so the card doesn't collapse when the ticket pins out, and runs the clearPane effect (see "Containing-block guards"). The rest→fixed flip is scroll-coincident → seamless (no jump). Entry unifies: scroll docks · a tab click `scrollGlide`s to the dock point · ✕ `scrollGlide(0)` back to the card (no fixed→static hand-off → no end jerk). Switching tabs while pinned keeps scroll position.
  - **The condense only animates on REAL scroll.** The width/padding/title-size interpolation is a CSS transition that only fires when wheel/trackpad scroll crosses the edge over several frames. An instant programmatic `scrollTo` jump bypasses the transition (the state flips in one frame) — so preview/instrumentation that measures mid-flight width after a jump reads unreliable values; the SETTLED end-state values are the correct ones.
- **Condense — definite width that interpolates, flex content-hug centred, in-flow close.** The condense animates as one calm settle: the `.bench-ticket` transitions `width`/`padding`/`box-shadow` on `--dur-settle`/`--ease-snap`, and the frame `padding`/`border-width`, tab title `font-size`, eyebrow `opacity`, and close `opacity` all transition on the same beat so they read as a single gesture.
  - **The condensed width is DEFINITE (`290px`), not `fit-content` and not a percentage** — width can only interpolate between two definite lengths, so the transition runs a clean 460→290. `fit-content` was tried and rejected: intrinsic sizes don't animate, so the box snapped/jerked instead of gliding.
  - **The rest tabs are a grid (`1fr auto 1fr`, `padding:0 30px`); the CONDENSED tabs switch to flex content-hug** — `display:flex; justify-content:center; gap:var(--space-24); padding:0 var(--space-24)`, with each item `flex:0 0 auto` (label = its own width, so the gaps measure to the labels). `justify-content:center` makes the box shrink CENTRED, so when the definite width is tuned to the content the leftover slack lands at 0 and all FIVE spaces end exactly even: edge · Visual · divider · Longform · ✕ · edge.
  - **Even gaps by construction.** ONE token (`--space-24`) is simultaneously the inter-item `gap` AND the left/right edge `padding`; the definite `290px` width is tuned so centred slack → 0. That is the entire mechanism — don't try to balance gaps by hand. **Gaps must stay on the within-element scale (`--space-24` ceiling, per `docs/rhythm.md`); pulling a 32+ structural token for inner gaps is drift.**
  - **The close is IN-FLOW when condensed (`position:static`, a real flex item), absolute + `opacity:0` at rest.** This is deliberate, NOT the old "out-of-flow forever" design — it has to occupy a flex cell so the five gaps are even. At rest it's `position:absolute` (right edge) with `opacity:0`/`pointer-events:none` so it reserves no rest-state space.
  ...
  - Don't revert the condensed tabs to grid, don't make the condensed width `fit-content`/`%`, don't pull the close back out of flow when condensed, and don't bring back an eyebrow max-height collapse.
...
- Click any tile (or its caption dot) → tile gets `.is-active`, lifts via `translateY(-12px) scale(1.025)`, siblings recede to MONOCHROME via the shared **`--backseat-dim`** filter (`.sc-grid.is-dimming .sc-piece:not(.is-active)` — the same backseat recede as the biconomy/rr menu-open dim; a stepped-back desaturate, not an `opacity`/`saturate` fade). On desktop / tablet, `SpecNote` renders inline beside the frame on the side decided at activation by `ShowcasePiece` (column-position measurement → `left` or `right`). On mobile, `SpecNote` is rendered once by `ShowcaseBottomSheet` as a singleton portal at `document.body` (see §"Index card placement rule" below).
...
The Visual tab's `.sc-section` (rendered by `Essay/WorkPanel.tsx` as `.sc-section bench-showcase`) is capped at `max-width: 1224px` and centred with `margin-inline: auto`, so the grid sits on the same centred rails the bench card uses on wide viewports rather than anchoring to the workbench's left padding. Below ~1352 px the cap resolves to 100% and the tablet/mobile media queries take over.

> **Removed (bench redesign):** the first-view scroll cue (`.sc-cue` + `FirstView.tsx` + the `--sc-cue-*` measured vars + `data-cue-v/h-anchor` + `SHOWCASE_DOCK_ID`) and the `.selected-firstview` / `.selected-layout` two-column stage (incl. its `min-height: 777px` canvas floor and the AboutCard/nav-row chrome). The bench ticket/tab navbar replaced the first-view hand-off; those mechanisms no longer exist. The Cases tab mounts `SelectedContent` directly in `.bench-cases`.
