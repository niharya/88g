...
This file is the protective archive for `/all`. The compressed digest lives in `./CLAUDE.md`
(auto-loaded); this archive carries the why. **How to read: grep the heading named by the
digest's pointer and read only that section** — the Index below is the cheap map.
...
- **Entrance — the gate-release settle** — the cold-load card settle, and the arrival gate that keeps it off the other two entrances.
- **Press states — condensed touch targets** — tap feedback scoped to `.is-condensed` only.
- **Containing-block guards** — what keeps the fixed ticket from getting trapped by a transformed ancestor.
- **Viewport-driven 3:4 card** — the invitation card's height-driven width + relative scale cap.
- **Bench-exit +Nihar marker — the shared ReturnMarker primitive** — why the exit arrow points left, by specificity not source order.
...
- **Mobile cases (MobileCases.tsx + CasesSheet.tsx)** — the separate mobile composition + inline (non-portaled) sheet.
- **Tablet cases layout** — why the stale tablet `@media` block was removed.
- **Index-card copy split + dev editor** — `card-copy.ts` as the single source of truth for reader-facing copy.
- **Filter strip** — the Visual-tab category radiogroup and its pure-CSS recede/emphasis.
- **Layout idiom — 9-col CSS Grid with JS-measured row spans** — the bento grid's span math and state-driven row spans.
- **Click + focus interaction** — tile open/close, stacking, and z-index contract.
- **Spec note** — the index-card layout, spacing, and per-piece colour cascade.
- **Caption content** — the tile caption shows dot + type + year only, no project name.
- **Bento reading order** — the `num`-driven reading order and the cardstack/furrmark swap.
- **Hint pill — neutral shell across all cards** — the "opens in new tab" pill stays a neutral shell, themed text only.

Historical (retired archive panel — kept for archaeology, not load-bearing on the live route):
- **Archive timeline model**, **Spacing system**, **Main timeline positions**, **Now dot — time-of-day shapes**, **Typography**, **Spacing grid**, **Icons**, **Pending migration — `.ap-entry__hint` → `<Monostamp>`**, **Component ownership**, **Animation: train metaphor**, **Hover system**, **Recipe: adding a new project**, **Don't-touch items**, **Responsive anomalies (mobile ≤767px)** — document the retired `ArchivePanel`/absolute-coordinate timeline. Read only if working archaeology, not for the live route.
- **Stale-hover gate — `data-armed` on ProjectCard** — still current; the gate mechanism `ProjectCard.tsx` relies on today.
...
- The container-query scaling mechanism (why the container lives on `.bench-stage`, not `.bench-card`) is its own entry — see "`--bu` container-query spine" below.
- The condense morph (rest↔pinned width/tabs/close interpolation) is its own entry — see "Condense" below.
- The condensed-state press feedback is its own entry — see "Press states — condensed touch targets" below.
- The fixed-ticket containing-block guards are their own entry — see "Containing-block guards" below.
- Deep-link entry and default-tab selection are their own entry — see "Deep-link entry & tab order" below.
...
**What it is.** TransitionSlot's exit-dim selector includes `.bench-workbench > *`.

**Where.** TransitionSlot's ghost-dim CSS (shared shell layer, not route-local).

**What breaks.** Renaming or dropping the `.bench-workbench` wrapper class means the outgoing `/all` ghost no longer dims on exit — the cross-route transition reads as a hard cut instead of a fade.

## `--bu` container-query spine

**What it is.** Every dimension on the card + ticket is authored as `calc(N * var(--bu))`, a real **layout** scale (line-breaks and authored proportions hold, text crisp). `--bu` ("one baseline pixel") = `calc(100cqi / 820)` — 1px at the 820px scale-1 baseline expressed as a fraction of the query container's width.

**Where.** **The query container is `.bench-stage`, NOT `.bench-card`** (`container-type: inline-size` on the stage).

**Why.** The stage has no border/padding, so its content box == the card width (`--bench-card-w`); putting the container on `.bench-card` is circular — `cqi` → the card's content box → minus the card's own `--bu`-based padding → a feedback loop that resolves too small (observed: ~0.53× instead of 0.75×). At the 615 cap, `--bu` = 0.75px, so `24 * var(--bu)` = 18px etc. — reproducing the old `--card-scale: 0.75` render exactly, and scaling proportionally below. This SUPERSEDES the old JS `--card-scale` knob (a `BenchEssay` resize handler), now **removed**. It is NOT `transform: scale()` — a transform would make the card the containing block for the fixed docked ticket and break the navbar (see "Containing-block guards"); `container-type: inline-size` does NOT trap the fixed ticket (verified — it still pins to the viewport, and `--bu` resolves to the stage width even on the fixed ticket).

**What breaks.** Moving the container back onto `.bench-card`, or swapping to `transform: scale()`, both re-break the math or the fixed ticket. Tune via the `--bench-card-w` clamp on `.bench-stage`.

## Condense

**What it is.** The docked ticket's rest→pinned width/tabs/close morph. The condense animates as one snappy settle on **`--dur-slide`/`--ease-snap`** (was `--dur-settle`): the `.bench-ticket` transitions `width`/`padding`/`box-shadow`, and the frame `padding`/`border-width`, tab title `font-size`, eyebrow `opacity`+`translateY`, close `opacity`, the divider, and the bead all ride the same `--dur-slide` beat so they read as a single gesture.

- **REST width is DEFINITE, `calc(460 * var(--bu))` + `max-width:100%`** (NOT `width:100%`). **This SUPERSEDES the old "`max-width:460` is load-bearing / the dock explodes the ticket" item.** That bug came from `width:100%`: once the ticket pinned (`position:fixed`) the percentage re-resolved against the **viewport** (~1433px) and the width transition animated FROM there, so the ticket visibly exploded toward full width before snapping in. A *definite* px width can't explode — and because it's px→px from the first frame, the width transition starts immediately at the pin (no re-resolution hold) so width + height collapse in **lockstep**. The definite width IS the guard now; `max-width:100%` only keeps it from overflowing a narrower container (mobile).
...
- **`--cu` — the condensed scale unit. Inside `.is-condensed`, size against `--cu`, NEVER `--bu`.** The ticket is card content at REST (rides `--bu`, correctly — its box scales too) and viewport-pinned CHROME when condensed, where the box is a fixed 236px. `--cu` is declared on `.is-condensed .bench-ticket` as `0.75px` — `--bu` frozen at the value it holds at the 615 card cap this ticket was tuned against — so every authored number is unchanged (`gap: calc(20 * var(--cu))`, `padding: 0 calc(20 * var(--cu)) 0 calc(24 * var(--cu))`, tab `padding: calc(12 * var(--cu)) 0`, title `font-size: calc(20 * var(--cu))`). **This is a units fix, not a retune**, and expressing it as a unit rather than as computed literals (15/18/9/15) is deliberate: it keeps the authored numbers visible, names the actual design fact, and makes the rule structural — the next condensed metric someone adds reaches for `--cu` because that is what the surrounding condensed CSS uses, instead of reaching for `--bu` and silently reintroducing the bug.
- **What `--bu` inside the condensed box actually broke.** The box width (236), the sub-labels (10px), the divider (2px) and the ✕ glyph (18px) were always px; the tabs' gap + edge padding, the tabs' vertical padding, and the condensed title were not. `--bu` tracks **viewport HEIGHT** (`--bench-card-w` → cqi → 0.62→1.0 across desktop), so the inner spacing grew while the fixed box and fixed labels did not: past `--bu ≈ 0.84` (viewport taller than ~1025px — any maximised 1080p/1440p window) the row overflowed the 233px of inner width, flex crushed `.bench-perf` 2px→1px, and `justify-content:center` + the ticket's `overflow:hidden` clipped the surplus off BOTH edges, leaving ~9px left / ~5px right instead of the intended ~23px. Measured at 1440 wide: 900px tall → 25/23 ✓ · 1180px tall → 9.5/5.5 ✗. After `--cu`: identical at `--bu` 0.64 / 0.98 / 1.0 and at 375px mobile — 236px box, 50.3px row, 25/22 margins, 2px divider, 15px title. The title matters most because it decides each tab's WIDTH once the box is fixed: on `--bu` it overtook the 10px sub-label above `--bu ≈ 0.9` and pushed the row past the box. Both ends of the `font-size` transition stay definite lengths, so rest→condensed still interpolates.
- **HEADROOM — the box is copy-dependent, and it fails SILENTLY.** Content 189 (tabs 50 + 74, divider 2, ✕ 18, three 15px gaps 45) + padding 33 = 222 in 233px of inner width → **11px of slack**, which `justify-content:center` splits, landing 23.5/20.5 inner + the 1.5px border = the 25/22 you measure. Grow the sub-labels past ~11px total and `overflow:hidden` clips the row again with no error — re-run the arithmetic (or measure `getBoundingClientRect`) on any copy change to "showcase" / "case studies". The 10px mono sub-labels, not the serif titles, are what set the tab widths at `--cu`.
- **Known limit, accepted:** every metric in the condensed navbar is a definite length, so it ignores the user's browser *font-size* preference (browser *zoom* scales it normally). The 10px sub-labels always behaved this way; the `--cu` freeze extends it to the rest of the box. The alternative — a content-sized box — can't animate its width, which the rest→condensed morph requires.
- **Condensed tabs' left padding is authored asymmetric (15px right, 18px left) on `.is-condensed .bench-ticket__tabs` — an OPTICAL correction, not a box error.** Confirmed via live DOM measurement (`getBoundingClientRect`) that the structural padding was already numerically symmetric (~24px both sides at 1440px) — it still read left-heavy because the left tab's serif title glyph (Fraunces, `-0.4px` letter-spacing) has almost no left side-bearing so its ink hugs the true box edge, while the right-side close (`×`) icon carries visible internal glyph padding, making the right margin look more generous at equal box padding. Fix is a hand-tuned dial value (same family as the landing caption tuck's `-4px`, see `app/_landing/ANOMALIES.md`) — **do not "fix" this back to symmetric padding**; that reintroduces the left-heavy read. Re-verify by eye (not just by measuring the box) before touching it.

**What breaks.** Don't revert the condensed tabs to grid, don't make either width `fit-content`/`%`, don't pull the close back out of flow when condensed, don't bring back an eyebrow max-height collapse, and don't put any condensed-state metric back on `--bu`.

## Entrance — the gate-release settle

**What it is.** On a cold load `/all` had **no entrance at all**: the page gate (`globals.css` → Page gate) holds `.workbench` at `opacity:0` behind the `.page-boot` loader and then flips it to `1` on `html.fonts-ready` with a plain opacity transition — the broadside simply appeared, already placed. `docs/navigation-choreography.md` §5.1 names this exact gap ("Reveal = Place, not a flat flip") as a Phase-1 item that was never built for this route. `.bench-card` now glides up 18px and settles as ONE object over `--dur-glide`/`--ease-paper` (the "mat glides" beat) as the gate releases.

- **Transform ONLY — no opacity in the keyframe.** The gate already owns the fade; a second one here double-fades. Delayed by `--delay-reveal-handoff` so the settle starts on the same frame as the gate's opacity transition, letting the loader lead its lift-away.
- **`backwards` fill, never `both`/`forwards`** — same discipline as `bench-slide-in`, same reason: the steady state must retain NO transform on `.bench-card`, which would become the containing block for the ticket's `position:fixed` morph and pin the navbar to the card instead of the viewport. This is guard (3) in "Containing-block guards", now covering two entrances. Verified after the animation finishes: `.bench-card`, `.bench-card__main`, `.bench-stage`, `.bench-workbench` all compute `transform: none`, and the docked ticket still resolves `position:fixed` at `top:16` against the viewport.
- **`.bench-workbench--settle` is an ARRIVAL GATE, not a style hook — and the test is ONE POSITIVE FACT.** Three arrivals land on this page and only one has no entrance of its own: cold load / reload (the flat gate fade), landing → `/all` (`SlideInOnNav`'s lateral `--slide-in`), and works ↔ works (TransitionSlot's Exchange). `useBenchDock`'s mount layout-effect arms the class only when **the page gate is still HELD at mount** — i.e. `.fonts-ready` is not yet on `<html>`, which is true of a cold load and of nothing else. Both soft navs run inside an already-released document, so the class never arms for them. The class only *arms* the animation; `html.fonts-ready` in the selector is what *starts* it, so the card settles as the gate releases rather than at mount. **Pure CSS cannot replace the arm step** — `.fonts-ready` persists across soft navs, so a selector keyed on it alone fires on all three.
- **REJECTED — surveying the other two entrances instead of testing the gate.** The first implementation asked "has anyone else claimed this arrival?" (`root.classList.contains('bench-workbench--slide-in')` + `document.querySelector('.workbench.transitioning')`). The `.transitioning` half is **structurally broken, not merely mistimed**: React runs CHILD layout effects before PARENT ones, and TransitionSlot is the parent, so this hook always runs before `.transitioning` is set. Verified in production — `/rr → /all` armed `--settle` and ran `bench-settle-in` on top of the Exchange. The `--slide-in` half worked but only by accident of JSX ordering (`<SlideInOnNav>` above `<BenchEssay>` in `page.tsx`), a load-bearing invariant nothing enforced. The gate test replaces both: no ordering dependency, nothing to ask TransitionSlot, one condition instead of two negatives. **Don't reintroduce either check.**
- **The one case where both classes CAN coexist — benign, verified.** A hard load of `/all` while a stale `nav-direction: to-bench` sits in `sessionStorage` (a landing → `/all` click that was abandoned before `SlideInOnNav` consumed the flag) arms `--settle` (gate held) *and* `--slide-in` (flag present). The card does NOT double-animate: `html.fonts-ready .bench-workbench--settle .bench-card` (0,3,1) out-specifies `.bench-workbench--slide-in .bench-card` (0,2,0), so the settle wins — which is the right entrance for a hard load anyway. Verified live: card ran `bench-settle-in` only. The only artifact is `.bench-exit`, which the slide-in rule matches alone, so the marker slides laterally while the card settles. Rare, cosmetic, and self-correcting on the next navigation — not worth reintroducing a class survey to guard.
- **The cold-load timing the gate test relies on.** Hydration (and therefore this layout effect) runs when the JS bundle executes; `.fonts-ready` is landed by the inline script in `layout.tsx` on `window.load` AND `document.fonts.ready`, both of which resolve after that. So on a cold load the class is reliably absent at mount. Verified on a production build across all three arrivals: cold `/all` → `--settle` + `bench-settle-in` at `translateY(18)`; `/rr → /all` → no class, no animation; landing → `/all` → `--slide-in` + `bench-slide-in` at `translateX(40)`.
- The bench is deliberately **not** a `Sheet`/`useReveal` consumer (`DESIGN.md` → "Idiom"), so this is the bench's own entrance expressed in the shared motion vocabulary — not a `section-reveal`, and it must not be unified into one.
- Reduced motion zeroes it, alongside the lateral slide.
- On the CSS-only gate failsafe path (`page-gate-failsafe` at `--dur-gate-cap`, when the release script never runs) `.fonts-ready` never lands, so the settle never fires and the card just appears. That is the existing degraded path for every surface — acceptable, not a regression.

**What breaks.** Don't switch the fill mode to `both`/`forwards` (traps the fixed ticket), don't add opacity to the keyframe (double fade), don't drop the `--settle` class in favour of a pure `html.fonts-ready` selector (fires on all three arrivals), and don't replace the gate test with a `.transitioning` or `--slide-in` survey (see REJECTED above).

## Press states — condensed touch targets

**What it is.** The three condensed tap targets get a tactile settle, scoped to `.is-condensed` only (micro tier, `--dur-instant`/`--ease-snap`, no bounce, no pill): `.is-condensed .bench-tab:active` → `scale(0.95)` + ink deepen + accent underline; `.is-condensed .bench-ticket__close:active` → `scale(0.88)` + colour deepen.

**What breaks.** Don't extend these to the rest (invitation) state — the press cue belongs to the navbar affordance, not the broadside.

## Containing-block guards

**What it is.** The docked ticket is `position:fixed`; a transformed/will-change ancestor would pin it to that ancestor instead of the viewport, so it scrolls away. Three guards keep the containing block clean: (1) `.transition-pane:has(.bench-workbench){will-change:auto}` in bench.css (the shared pane carries `will-change:transform`); (2) `useBenchDock` cancels the pane's retained WAAPI entrance transform (TransitionSlot's `fill:both` leaves `translateY(0)`) once `.workbench.transitioning` clears; (3) BOTH `.bench-card` entrances — the landing slide-in AND the cold-load settle — use `backwards` fill so the card retains no transform (see "Entrance — the gate-release settle"); any third entrance added here must do the same.

**What breaks.** Don't reintroduce any retained transform on a ticket ancestor — the fixed navbar re-breaks and scrolls away with the pane.

## Viewport-driven 3:4 card
...
## Bench-exit +Nihar marker — the shared ReturnMarker primitive
...
**What it is.** Every timeline size is `calc(N * var(--bu))` so the whole sheet scales as one unit. Type design sizes (rigorous, all `N * var(--bu))`): card title `24·--bu` Fraunces, body `12·--bu`, compact-card title `14·--bu`, ALL mono `10·--bu` (unified — no 11px monos anywhere in the timeline). The `t-h5`/`t-btn1` shared token classes (greeting/role/nameplate) are used as-is, not re-scaled by `--bu`.
...
**What it is.** Hovering one case study eases the OTHER two AWAY (one above drifts ↑, ones below ↓) via `:has()` on `.selected-tl`. `6·--bu` throw.

**Where.** The translate rides the **card** (`.project-card--compact`, a non-motion `<a>`), NOT the row.
...
- **Row-spans are STATE-driven** — `--sc-rowspan` is set from React state in `Showcase.tsx`, NOT via imperative `style.setProperty` — because a re-render would wipe an imperative value. The rAF measurement cleanup must **null `rafRef`**: a stale id makes the next measure bail forever, which shows up specifically under React Strict Mode's double-invoke. **Fallback span is 18** (≈756px), not 30 — if a measurement fails before layout settles, 18 is the safe default row count.
...
- **Energy pips (v2 gauge) have no Figma source.** The continuous masked energy bar was replaced by 12 pips in groups of 4 (one pip per energy point). The update arrived as a PNG export only, so pip size (9.3 × 14.6), in-group gap (3.1), radius, and the lit/slot colours are *measured* off that export (it is 2× our units), not copied from Figma values. The pip track deliberately keeps the retired bar's 157.968 × 13.164 box — the pips overflow it evenly, which is what holds the panel's height and the pips' position to the design. Don't "fix" the overflow by growing the track.
...
## Hint pill — neutral shell across all cards

**What it is.** The "opens in new tab" hint pill (mint/external cards on the current timeline — `ProjectCard`'s mint variant and the `<ProjectCard compact>` children) keeps a neutral grey shell, with only the text color themed per project.

**Where.** `ProjectCard` mint styling; carried over from the original `.ap-entry__hint` pattern (see "Pending migration" below for that historical instance and its `<Monostamp>` migration plan).

**Why.** Theming the shell itself (background/border) per project reads as visual noise across a page with several projects; theming only the text keeps the pill legible and quiet while still tying it to its card.

**What breaks.** Don't theme the shell — only the text color.
...
## Responsive anomalies — mobile pass

A cohesive mobile pass across the whole route (intro card + timeline + showcase). The through-line: **one relative sizing language (cqi/`--bu`) and one shared 24px gutter** on mobile, plus a purpose-built showcase "open" interaction. Breakpoint is the shared clause `(max-width: 767px), (max-height: 500px)`.

### Intro card + shell (bench.css, globals.css)

- **Mobile `--bu` re-base + cap.** On mobile `.bench-stage` overrides `--bu: min(calc(100cqi / 460), 0.62px)`. The desktop 820 baseline resolves `--bu ≈ 0.38` at phone width → poem ~9px (unreadable) — which is why this block *used* to hardcode px. Re-basing to 460 lands the poem ~15px and the whole card scales as ONE cqi unit (no fixed-px exception). The `0.62px` **cap** stops a landscape phone (wider stage via the max-height clause) ballooning type to ~32px — it widens the column instead. Don't reintroduce fixed-px sizing here.
- **`.bench-card` aspect-ratio → `auto` on mobile** (content-driven height). A phone portrait can't hold a legible 3:4 at that width — the retained ratio was clipping the poem. The framed-sheet read comes from the border + lift, not the ratio.
- **`.bench-ticket__tabs` rest padding trimmed to `calc(16 * var(--bu))` on mobile** (from 32·--bu). At the re-based (larger) `--bu` the two tab titles don't fit the narrow frame at 32·--bu. Rest state only — the condensed 236px row keeps its own padding.
- **`.bench-work` drops its 8px horizontal padding on mobile.** The intro card sits on the workbench's 24px pad only; the tab content (timeline + showcase) is one level deeper inside `.bench-work` which adds another 8px, so it was inset 32px vs the card's 24px. Zeroing bench-work's side pad aligns the whole page to ONE 24px gutter.
- **Global mobile `--sheet-gutter: 24px`** (`app/globals.css`, mobile `@media` on `:root`). Narrows every mobile sheet (card, timeline mat, showcase objects) to the same edge with one knob. **Safe globally only because the landing's framed sheet is desktop-only (`min-width:768`)** so its mobile view never reads the token — verify that before "simplifying."
- **Docked nav ticket recedes when a showcase artifact is open.** `html.sc-artifact-open .bench-ticket { opacity:0; box-shadow:none; pointer-events:none }`. `Showcase.tsx` toggles `sc-artifact-open` on `<html>` (effect keyed on activeId + isMobile) because the docked ticket lives in the bench shell, OUTSIDE the grid subtree — CSS can't reach it from the grid, so the signal routes through the document root. `opacity` was added to `.bench-ticket`'s transition for the fade. Don't drop the class toggle or the transition property.

### Timeline / cases (selected.css, MobileCases.tsx, CasesSheet.tsx)

- **FOOTGUN — the mobile timeline mat is an edge-to-edge graph-paper surface, and the grid is a `background-image` LONGHAND on the shared `.mat` class.** `.selected-mat` mobile bleeds to the true viewport edge (`width: calc(100% + 2*var(--workbench-pad-x))` + negative margins; reaches 0 because bench-work drops its side pad), no border, no shadow. **Never set a `background:` SHORTHAND on `.selected-mat`** — the shorthand resets `background-image` and wipes the grid (the exact footgun documented in `app/_landing/ANOMALIES.md`; it bit again here — an intermediate transparent-mat version made the grid vanish). Only `background-color` longhand. The cards inset within it via `.cases-mobile` padding.
- **MobileCases internals on the cqi/`--bu` spine**, baseline 327 (the card column at a 375 phone → `--bu`=1, so `N` reads as the design px), capped 1px; `container-type: inline-size` on `.cases-mobile`. Horizontal card inset stays the fixed sheet gutter; vertical rhythm + type scale on `--bu`. 1px hairlines + token classes (`t-h5`) stay fixed.
- **MobileCases foot uses the shared `<NavMarker>`** (role=chapter, category/title icon, wipHint), same as the desktop nameplates — not hand-rolled markup. The old `.cases-mobile__foot-row` `<button>` showed a stray UA border (black box) because the CSS only set `border-top`. Only the stacking + top-divider is route-local (`.cases-mobile__foot > .nav-marker`).
- **CasesSheet restyled to match the showcase SpecNote sheet** — squared narrow docked card at `--sc-obj-w`, mint frame on top + sides (no bottom border), upward lift shadow; its own scrim + translateY reveal kept. Visual match only, no shared primitive extracted (deliberate).

### Showcase open interaction (showcase.css, Showcase.tsx, ShowcasePiece.tsx, ShowcaseBottomSheet.tsx)

- **Tile `:hover` rules gated behind `@media (hover: hover)`.** On touch an ungated `:hover` makes the FIRST tap apply hover (no open) and only the second tap opens (the iOS two-tap). The two rules that combined `:hover` with `.is-active` (cap-dot, reduced-motion transform) were SPLIT so `.is-active` still applies on touch — don't re-merge.
- **`--sc-obj-w` (standard mobile object width) is on `:root`, not `.sc-grid`** — the info card portals to `<body>` and must read it from outside the grid subtree. Mobile also resets the desktop wide-tile inset (`.sc-slot--wide .sc-piece { max-width:100% }`) so paymaster/ecochain fill the column.
- **Mobile gap `--sc-gap: 40px` is MIRRORED by `MOBILE_GAP_PX` in `Showcase.tsx`** — the JS row-span measurement uses it; change both together or the masonry drifts.
- **Scroll-to-dismiss, not a hard body lock.** iOS `overflow:hidden` is unreliable and a position:fixed freeze fights the open scroll. Instead a page scroll past a viewport-relative threshold (~6% vh) closes the sheet + resets the grid. The open's OWN programmatic scroll must not self-dismiss, so the dismiss ARMS only after that scroll settles (debounce-settle: each scroll event restarts a short timer; arm when scrolling stops). The old `ShowcasePiece` mobile scroll-into-view was removed (it fought this); the scroll now lives in the sheet. `closeSheet` is a stable `useCallback` so the baseline is captured once.
- **Open choreography (mobile).** The active tile nudges DOWN (`.sc-piece.is-active` transform overridden — not the desktop lift-up); its IMMEDIATE top+bottom neighbours fade via `:has()` sibling selectors (`.sc-slot:has(.sc-piece.is-active) + .sc-slot` and `.sc-slot:has(+ .sc-slot .sc-piece.is-active)`). Relies on the single-column mobile grid ⇒ DOM order == visual order. This block is placed LAST in showcase.css so its active-transform override beats both the base lift and the earlier reduced-motion reset; it carries its own nested reduced-motion guard.
- **Sheet centred via `margin-left: calc(-0.5 * var(--sc-obj-w))`, NOT transform** — transform is left free for the `sc-sheet-rise` animation. SQUARED (no border-radius — no rounded corners anywhere on this site), no bottom border (runs off the bottom edge), generous bottom padding buffer.
