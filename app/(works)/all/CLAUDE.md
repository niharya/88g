...
- The condensed ticket width is a DEFINITE `min(236px, calc(460 * var(--bu)))`, not `fit-content`/`%`; the CSS transition only fires on real scroll (a programmatic jump bypasses it) — don't revert the condensed tabs from flex to grid. ANOMALIES.md → "Condense"
...
- Deep-link entry (`/cases`, `/showcase`) reads bare query flags SERVER-side in `page.tsx` (client `useSearchParams` never sees the rewrite query) — moving the read client-side breaks tab selection. Default is Longform. ANOMALIES.md → "Deep-link entry & tab order"
