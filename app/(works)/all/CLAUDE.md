...
- The soft-nav Hold is a DEPARTURE marker (landing `markToBench` mounts, `useBenchDock` removes) — no `loading.tsx` here, and never one at the (works) group level (it rides TransitionSlot, breaking the first-sheet `.revealed` handoff). ANOMALIES.md → "Route hold — a DEPARTURE marker, not a loading boundary"
