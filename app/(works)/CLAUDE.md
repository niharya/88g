...
- The lift ESCALATES to the loader Hold only on stall: no commit within `STALL_MS` mounts the same `#route-hold-departure` marker markToBench uses (shared id = shared release wiring: /all arrival, commit via `clear()`, 8s failsafe, pageshow/popstate). A commit inside the window never shows the loader — the Exchange stays the works idiom. Rationale in departureLift.ts's header ("STALL ESCALATION").
