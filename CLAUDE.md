...
* **Branch digests** — nested `CLAUDE.md` files in every protected area (`app/`, `app/(works)/` + each route, `app/marks/`, `app/shape-of-product/`, `app/_landing/`, `app/components/`, `app/components/nav/`). Claude Code auto-loads a digest the moment any file in its folder is touched, so protection is structural, not memory-dependent. Each carries a standard header (family line, archive pointer, maintenance rule) plus one-line don't-touch items — one line each, capped ~1500 words total, in the fixed grammar `<guard> — <what breaks>. ANOMALIES.md → "<Heading>"`. The digest states the constraint; it never re-narrates the archive.
* **Archives** — per-area `ANOMALIES.md`: the full protective memory (rationale, what-breaks, rejected approaches), organized as anchored `## <Heading>` sections with a `## Index` at the top. **Never read a whole archive.** Grep the heading a digest line points at and read only that section — same discipline as `LIBRARY.md`. For anything wider than one section (a structural change touching several guards), send `route-auditor` instead of reading the file yourself; it's read-only and reports back only what conflicts.
...
* **Analyze before you build.** The digest auto-loads when you touch a protected area; before structural changes, follow each relevant digest line's archive anchor (grep the heading, read only that section — or send `route-auditor` for a wider check), grep `LIBRARY.md` before building anything that might exist, read only the closest matching pattern, present a short plan-to-system mapping, and wait for confirmation. Do not skip this.
...
**Production is a different machine from `next dev`.** Response headers, redirects and caching exist only on the deployed site, so a whole class of bug is invisible to every local check — `/resume` shipped blank for 47 releases that way. Security headers are declared in `next.config.mjs` (source of truth; `netlify.toml` holds an identical copy for CDN-served files) — see `LIBRARY.md` → "Security headers". `npm run smoke` asserts the live site and is Phase 6 of `/release`; if a change depends on anything the server adds, it isn't verified until that has run. `npm run prod:check` (pre-push) pins the other two production-only invariants: every page route prerenders static, and the `next.config.mjs`/`netlify.toml` header twins stay identical.
