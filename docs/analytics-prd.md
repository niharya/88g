...
- **2026-06-25 — Batch 5 (v0.110.0): reverted the first-party proxy — it broke geolocation.** The Batch 4a `/_stats/*` proxy routed beacons through Netlify, so Umami saw Netlify's server IP (Singapore region) and geolocated EVERY visitor there. Reverted to direct: script from `cloud.umami.is`, beacons to `gateway.umami.is`; CSP re-adds those hosts (`cloud.umami.is` script-src, `gateway.umami.is` connect-src); `data-host-url` + the `[[redirects]]` removed. Trade-off accepted: ad-blockers can catch direct analytics (some undercount), but visitor locations are now accurate — which matters more for a portfolio. Getting both cleanly needs Umami Cloud's paid custom-domain (CNAME).
- **Pending:** deploy via `/release`, then confirm on the live site — a page view in the dashboard with the **correct location** (not Singapore). Optional later: promote CSP from report-only to enforcing once `/csp-report` shows it's clean. The per-visitor "adaptive" track (Track B) stays parked.
