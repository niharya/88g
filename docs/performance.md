...
- **Page gate (hold-until-ready).** Top-level surfaces (`.landing`, `.workbench`, `.route-marks`, `.route-sop`) carry `opacity: 0` until `<html>` gains the `.fonts-ready` class. **`.landing` is the exception: it is gated on OPACITY ONLY**, outside the shared rule group with its own `page-gate-failsafe-landing` keyframe — the shared failsafe's `pointer-events: auto` laid an invisible sheet over the Startooth canvas, and an animated value outranks any declaration trying to win it back. Never fold it back in. Nor is the gate class guaranteed: a React hydration mismatch re-renders the root, re-applies `<html className>` and strips `.fonts-ready` outright (`app/_landing/ANOMALIES.md` → "Clock-in-render wipes the page gate"). The gate script in `app/layout.tsx` adds the class when **both** `window.load` (the JS bundle + subresources) **and** `document.fonts.ready` settle — or an **8000 ms** failsafe ceiling (`GATE_CAP_MS` in `app/lib/gate.ts`, mirrored as the `--dur-gate-cap` CSS animation in `globals.css`) so a stalled asset can't trap the page. It holds for `window.load`, not fonts alone, because page **content** reveals via a post-hydration client hook (`useReveal` adds `.revealed`, running the section-reveal transition) that can't fire until the JS bundle has loaded — releasing on fonts alone shows an empty surface ~600 ms before hydration, leaving a blank gap. Below-the-fold images stay lazy (`<Img>` LQIP blur), so the gate isn't waiting on them. The `.page-boot` loader (a saturated per-route field + the Startooth mark) fills the screen during the hold, then on release the mark lifts off as the field fades. (`fonts-ready` is a legacy class name — it now means "page ready," not just fonts.)
...
- An **uncapped** JS font-gate (the pre-v0.56 behaviour with a 3 s ceiling and no failsafe). The current gate is bounded at **8000 ms** (`GATE_CAP_MS`) with a matching CSS-animation failsafe (`--dur-gate-cap`); keep the JS cap and the CSS failsafe equal. Don't gate on `document.fonts.ready` alone — content reveals post-hydration, so the page must hold for `window.load` too (see the gate note above).
