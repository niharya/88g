...
- **IMPLEMENTED (scoped) — the `.route-hold` mechanism, now a DEPARTURE hold.** The landing's Works click (`markToBench`, `app/page.tsx`) appends an invisible `.route-hold.route-hold--all` marker to `document.body` (veil-style, so it survives the route swap); `globals.css` (Page boot → Route hold) re-arms the root layout's *existing* `.page-boot` loader while it exists — `:root:has(.route-hold)` selectors out-specify the one-shot `html.fonts-ready` exit rules, the workbench holds hidden underneath, and on arrival `/all` removes the marker (`useBenchDock`) so the standard exit + the fonts-ready reveal transition replay: Hold → Place, same arrival as first load. An 8 s failsafe removes the marker if the navigation never lands; modifier clicks skip it. *Originally implemented as `app/(works)/all/loading.tsx`; moved to the click side when `/all` went static. The reason is the stall window, not a bug in loading boundaries: the reported 2–3 s of nothing happens BEFORE the navigation commits (client-chunk fetch against the canvas-saturated main thread), and a loading boundary only mounts at commit, so it structurally cannot paint during the window the complaint is about. A boundary was restored and rebuilt to test the stronger claim that Suspense breaks a statically generated route — it does not (route still `○`, page hydrated, deep links resolved, the marker appearing only in the RSC flight payload, never as a live DOM node), so that is recorded as tested-and-false in `app/(works)/all/ANOMALIES.md` → "Route hold". With `/all` static and fully prefetched, the commit-side window a boundary would cover is now the rare one, so the site keeps ONE mechanism, on the side the evidence pointed at.* Scoped to landing → `/all` deliberately: it is the only *cold* entry into the works shell; works↔works is warm via `TransitionSlot`'s mount-time prefetch (fully so now that `/all` is static), and keeping the hold out of those navigations keeps it out of TransitionSlot's ghost choreography (see `app/(works)/all/ANOMALIES.md`; the works EXIT now borrows it purely as a stall fallback — §5.6).
...
- **IMPLEMENTED — `acknowledgeOnClick='press'`, audit complete.** The marker keeps its pressed fill + 1px sink (`[data-departing]`) after a navigating click until the page unmounts (8s failsafe), so the click is acknowledged on the same frame even before the Hold can paint. Enabled on every plain-`<Link>` cross-page entry point: the landing Works marker, `ExitMarker` (works → `/all`), `NiharHomeLink` and `ReturnMarker` (→ landing). On the shell-less `ReturnMarker` the held fill has nothing to paint, so navmarker.css holds its hover ink + solid underline instead — same contract, flat vocabulary. Deliberately NOT adopted: the veil-driven crossings (`MarksExitMarker`, and the timeline's `/marks` marker) — `CrossShellVeil` starts dimming on the same frame, which IS their acknowledgment; same-page markers (they use `shake`/`morph`); the timeline project cards and the landing spectrum link (both already carry their own `:active` press); and the 404/error Home markers (rare path, base `:active` is enough). See `LIBRARY.md` → NavMarker.
...
- **§5.1 step 2 ("Reveal = Place, not a flat flip") — PARTIALLY LANDED (2026-08, `/all` only).** The bench's cold-load arrival runs a Place-equivalent instead of the gate's flat opacity flip: `.bench-card` glides up 18px and settles as one object on `--dur-glide`/`--ease-paper`, delayed by `--delay-reveal-handoff` so the loader leads its lift-away. It is deliberately **not** a `section-reveal` — the bench is not a `Sheet`/`useReveal` consumer (`app/(works)/all/DESIGN.md` → "Idiom") — so this is the route's own entrance expressed in the shared vocabulary, which is what §5.1 asks for without mandating the Sheet primitive. Transform-only (the gate still owns the opacity) and `backwards` fill (a retained transform on `.bench-card` would trap the docked ticket's `position:fixed` morph). Armed by a **gate-still-held test** in `useBenchDock` — `.fonts-ready` absent from `<html>` at mount, true of a cold load and nothing else — so it never stacks on the landing → `/all` slide-in or TransitionSlot's Exchange; surveying those two instead was tried and is structurally broken (React runs child layout effects before parent ones, so a `.transitioning` check always reads false). **Still unbuilt:** `/rr`, `/biconomy`, `/marks`, `/shape-of-product` and the landing all still flat-fade on cold load, so acceptance criterion §10.2 ("first-load and works→works entrances are visibly the same gesture") is not met site-wide; the rest of Phase 1 §5.1 and all of Phases 2–3 remain consciously not pursued. Full rationale + what-breaks: `app/(works)/all/ANOMALIES.md` → "Entrance — the gate-release settle".
...
- **D5** — RESOLVED (user), then **PARTIALLY REVISED (2026-08, field evidence)**: cross-shell transitions stay as-is (black veil everywhere, tone black by choice) and the durations stay documented-not-tokenised (D1). But the original "stall-loader is ~zero value on a static+prefetched site" verdict did not survive contact with production: the landing → `/all` click showed 2–3 s of zero feedback (the landing's canvas saturates the main thread through the intro, and route prefetch doesn't cover the client-chunk fetch), so the §5.5 stall-Hold and §5.6 click-acknowledge were built — scoped to that one cold entry (`/all` `loading.tsx` + the landing Works marker's `press` acknowledge), not as the full Phase 2 sweep. The rest of Phases 2–3 remains consciously not pursued. **Follow-up (2026-08): `/all` was the one route building ƒ dynamic** (a server `searchParams` read for deep-link tab selection, which capped prefetch at the loading boundary); the read moved to a client-side browser-URL resolution in `useBenchDock`, so `/all` now builds ○ static, prefetch covers the full payload, and the §5.5 Hold is genuinely the rare path (slow network / main-thread saturation), not a cover for a prefetch gap. Going static also moved the Hold's trigger from `loading.tsx` to the click-side departure marker (§5.5) — not because a boundary breaks a static route (that was tested and it does not), but because the stall the report was about happens *before* navigation commit, which is the one window a loading boundary can never paint in.

- **§5.6 extended to the works EXIT (2026-08).** Same report, second entry point: clicking EXIT on a case study read as dead. The held press was already there and is marker-scale; what was missing was page-scale feedback during the pre-commit window (~220ms locally, longer on a real device — TransitionSlot cannot move until React commits the new route). The Hold is the WRONG tool inside the works shell: `.route-hold` blanks `.workbench` on the click frame and would run the loader idiom straight through an Exchange that TransitionSlot already owns, and `useBenchDock` would strip the marker at commit anyway. So the principle transferred, not the mechanism — the click arms the Exchange's OWN first beat (Phase A, "content dims") on `.transition-slot` and TransitionSlot picks it up mid-flight at commit (`app/(works)/departureLift.ts`; `app/(works)/CLAUDE.md` → departure lift). Still scoped to the reported entry points, not the full Phase 2 sweep: `/all`'s case cards (works → project, going deeper) have the same pre-commit window and are deliberately left alone until they're reported. **Revised (2026-09): the Hold joined the works EXIT as a STALL ESCALATION** — departureLift mounts the same `.route-hold` marker only if the route hasn't committed within STALL_MS (700ms). The two objections above applied to a click-frame Hold and don't transfer: nothing blanks on the click frame (the Exchange stays the common path), and useBenchDock's strip-at-commit is the escalation's designed release, not a defect. See `departureLift.ts` → "STALL ESCALATION".
