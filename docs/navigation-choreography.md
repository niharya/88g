...
- **IMPLEMENTED (scoped) — the `.route-hold` mechanism.** `app/(works)/all/loading.tsx` renders an invisible `.route-hold.route-hold--all` marker; `globals.css` (Page boot → Route hold) re-arms the root layout's *existing* `.page-boot` loader while it exists — `:root:has(.route-hold)` selectors out-specify the one-shot `html.fonts-ready` exit rules, the workbench holds hidden underneath, and on resolve the marker vanishes so the standard exit + the fonts-ready reveal transition replay: Hold → Place, same arrival as first load. Scoped to `/all` deliberately: it is the only *cold* entry into the works shell (landing → `/all`); works↔works is always warm via `TransitionSlot`'s mount-time prefetch, and keeping the fallback out of those navigations keeps it out of TransitionSlot's ghost choreography (a stall fallback passing through the slot would run the transition against the loader and skip the first-sheet reveal — see `app/(works)/all/ANOMALIES.md`).
...
- **IMPLEMENTED (scoped) — `acknowledgeOnClick='press'`.** The marker keeps its pressed fill + 1px sink (`[data-departing]`) after a navigating click until the page unmounts (8s failsafe), so the click is acknowledged on the same frame even before the Hold can paint. Enabled on the landing Works marker; other nav entry points adopt it as audited. See `LIBRARY.md` → NavMarker.
