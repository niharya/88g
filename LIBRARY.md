...
- **ReturnMarker** — `app/components/ReturnMarker/`; flat (chip-less) back-link built on NavMarker(role="exit"); colour via `--return-marker-ink`/`-hover`; consumers: /all (BenchExitMarker), /privacy (PrivacyReturn).
...
The single nav-marker primitive — the shared shell behind every `ChapterMarker`, `ProjectMarker`, `ExitMarker`, and the landing's Nihar / Works markers. Emits the `.nav-marker` class structure that `nav.css` already consumes, so adoption is a visual no-op; tone / state / acknowledgment modifiers layer new behavior on top. Supports three element shapes (`a` / `button` / `div`) via a discriminated `as` union, three slot roles (`project` / `chapter` / `exit`), three tones (`neutral` / `terra` / `mint`), two states (`default` / `active`), and four click acknowledgments (`navigate` / `shake` / `morph` / `press` — press holds the pressed fill after a navigating click via `[data-departing]` so departure reads as registered while the destination arrives; modifier-clicks exempt, 8s failsafe release). Optional `wipHint` reveals a Monostamp chip beside the label on click (auto-dismiss after 8s). When the chapter marker docks, it + the adjacent project marker share a muted mat-coloured shell (`--mat-bg` + `--mint-100` border) so the pair reads as one unit.

**Where it lives**
- [app/components/NavMarker/NavMarker.tsx](app/components/NavMarker/NavMarker.tsx) — component, `NavMarkerProps` discriminated union, `NavMarkerRole` / `NavMarkerTone` / `NavMarkerState` / `NavMarkerAcknowledge` types, `useShakeState` + `usePressHoldState` + `useWipHintState` hooks.
- [app/components/NavMarker/navmarker.css](app/components/NavMarker/navmarker.css) — tones, press translate, press-acknowledge held fill (`[data-departing]`, per tone), shake keyframe (`nav-marker-arrow-shake`, 360ms, origin bottom), morph rotation (`.is-morphed` → 45°), `.nav-marker__wip.is-shown` chip reveal, and the docked fill rule covering the chapter marker and the adjacent project marker.
- [app/components/NavMarker/index.ts](app/components/NavMarker/index.ts) — barrel.
- Consumers: `app/components/nav/{ChapterMarker,ProjectMarker,ExitMarker}.tsx`, `app/page.tsx` (landing Nihar + Works markers), `app/(works)/all/page.tsx` (Works nameplate) + `app/(works)/all/components/Timeline.tsx`, the shared `NiharHomeLink` + `ReturnMarker` (own LIBRARY entries; ReturnMarker is the flat exit-link used by `/all` BenchExitMarker + `/privacy` PrivacyReturn), `app/marks/components/MarksExitMarker.tsx`, `app/shape-of-product/page.tsx` + `components/SopNavRow.tsx`. (This list drifts — `grep -rln "components/NavMarker" app` is the truth.)
...
- **Acknowledgment contract.**
  - `navigate` (default) — no visual click feedback; the route change is the feedback. Now the fallback rather than the norm: the cross-page entry points (landing Works, `ExitMarker`, `NiharHomeLink`, `ReturnMarker`) all set `press` per §5.6 of the choreography doc, so a click is never unacknowledged while a destination loads. `press` is MARKER-scale, though — it says "the click registered", not "the page is leaving". On a small fixed marker that has proved insufficient on its own (the case-study EXIT read as dead even with `press` shipped), so page-scale feedback rides in through the consumer's `onClick` alongside it: `ExitMarker` ← the works departure lift (`app/(works)/departureLift.ts`), landing Works ← `markToBench`'s route hold.
...
- **Props forwarded to NextImage.** `sizes`, `priority`, `draggable`, `unoptimized`, `loading`, `fetchPriority`, `prefetchMargin`. Anything else spread via `...rest`.
- **Never pass `onLoad`.** It is owned by `Img` — a consumer-supplied one lands in `...rest`, wins, and silently clobbers the internal load state so the image never reveals. Use **`onReady`** instead: a no-arg callback fired when the src is painted-ready, covering both the fresh-load path and the already-cached path (`loadedSrcs` hit at mount). `/biconomy` `Flows` uses it to know whether the after-image is in hand before letting the before/after toggle claim it swapped.
- **The reveal does NOT depend on `onLoad` alone.** React's synthetic `onLoad` only catches a `load` event dispatched after hydration attaches it, so an image that finishes first — eager assets below the fold, anything from cache — fires into the void and the fully-decoded image sits at `opacity: 0` forever. `Img` reconciles against `img.complete` on mount/src-change **and** attaches a native `load` listener as a backstop. Both halves are load-bearing; see `app/components/ANOMALIES.md` → "`Img`'s `onLoad` can miss the load event".
- **Near-lossless UI scans must bypass the optimizer (`unoptimized`).** `/_next/image` re-encodes to **lossy** WebP, and lossy WebP subsamples chroma (YUV 4:2:0) at *every* quality including `q100`. On photos and most artwork that's invisible; on screenshots of a UI — small text, thin rules, light-on-coloured fills — it haloes glyph edges. Measured on `/biconomy`'s flow scans: same 1988×1131, a **larger** file (127.6 KB vs 114.3 KB), visibly worse, max channel delta 61/255. `quality={100}` does **not** avoid this; only `unoptimized` does. The cost is losing `srcset`, so mobile pays full bytes — if that matters, pre-generate near-lossless variants at build time rather than reaching for the optimizer. `/biconomy` `BeforeAfter` (`FLOW_IMG_UNOPTIMIZED`) is the reference case; removing it shipped a regression in v0.130.0, restored in v0.131.0.
- **`fetchPriority` is the lever for competing images in one frame.** `sizes` decides *how big*, `loading` decides *when*, `fetchPriority` decides *who wins* when several images mount together. Any layout where an off-screen or `opacity: 0` image mounts alongside the visible one needs the visible one marked `"high"` and the rest `"low"` — otherwise the invisible images win on equal footing. The `/biconomy` audit frame is the reference case.
...
- [app/_landing/startooth-canvas.css](app/_landing/startooth-canvas.css) + [app/landing.css](app/landing.css) — `--sheet-width`; **page** archetype, viewport-fill (no cap); fixed-px content + a flat `--sheet-scale` readability nudge, not cqi yet (see notes).
...
- **Caps are model-specific — choose to the model.** The blue card is an aspect-locked card, so it uses a RELATIVE cap `min(80vw, 820)` to nest concentrically on iPads (a fixed cap froze it small). The landing sheet is VIEWPORT-FILL (fills the viewport minus a uniform `--sheet-margin`, content scrolls + the matte clips), so it carries NO width cap at all — the fixed-px hero column just centres on a broader field on large monitors. Same recipe, different cap posture because the height/width model differs; don't copy either cap onto the other.
- **The landing is a page member with fixed-px content on a viewport-fill sheet — no fit-to-frame scalar.** Its sheet grows to fill the viewport, so the fixed-px column always fits and needs no clamp. The old `--sheet-scale = min(1, frameWidth/760)` fit-to-frame bridge (with its JS resize/measure machinery) was REMOVED; `--sheet-scale` is now just a flat `1.02` readability nudge on desktop/iPad (`1` on mobile), still applied via `transform: scale()` on `.landing__content` and still gated to `@media (min-width:768px) and (min-height:501px)`. The tracked cqi/`--bu` conversion (every width, the self-adjusting cascade tops, paddings, type, AND the Group A/B settle motion → `calc(N * --bu)`) remains the motion-heavy follow-up if the transform caveats ever bite; the landing anomaly ("Sheet harmonization (Phase 3)") holds the current rationale.
...
The first-paint loader shown during the page-gate hold on hard refresh: a full-screen **saturated per-route field** (`--loader-screen-bg`) with the `StartoothLoader` cluster mark centered on it, so a confident branded screen holds even mid-load. The mark fades in ~`--dur-loader-appear` after mount (fast loads barely flash it); on `html.fonts-ready` the mark **lifts off** (`page-boot-mark-out`) as the field **fades** (`page-boot-out`). See [StartoothLoader](#startoothloader) for the mark.

**Where it lives**
- [app/layout.tsx](app/layout.tsx) — `<StartoothLoader>` inside a `.page-boot` div, sibling of `{children}` (server component, paints pre-hydration).
- [app/globals.css](app/globals.css) → "Page boot" — `.page-boot` field + fade/lift animations (`page-boot-in/out`, `page-boot-mark-in/out`), and `:root:has(<route-root>)` per-route `--loader-screen-bg` / `--loader-lit` / `--loader-sticker` overrides.
...
The postage-stamp ornament shown beneath an open `ProjectMarker`. Notched outer body with paper-edge wobble (inline `feTurbulence + feDisplacementMap`, scale 1) and a soft 1px drop-shadow; inner notched border traces 4px inset; 40×40 icon slot at left, hairline vertical rule, then a label stack (`lead` over `title`) — title wraps to as many lines as the slot allows. Tone (`olive | yellow | terra | mint`) swaps the gradient stops, stroke, and ink via four CSS custom properties. On every transition into the parent's `.marker-info-anchor.is-open` the stamp picks a fresh random tilt in [-2°, +2°] — observed via `MutationObserver` so `ProjectMarker`'s own contract stays untouched.

**Where it lives**
- [app/components/MarkerTicket/MarkerTicket.tsx](app/components/MarkerTicket/MarkerTicket.tsx) — component, `MarkerTicketTone` type, header comment with full contract.
- [app/components/MarkerTicket/marker-ticket.css](app/components/MarkerTicket/marker-ticket.css) — base layout, four tone palettes, mobile recompose. Imported once by [app/(works)/layout.tsx](app/(works)/layout.tsx) so every consumer route inherits it.

**AI notes**
- Tones drive **only** four CSS vars: `--marker-ticket-fill-a`, `--marker-ticket-fill-b`, `--marker-ticket-stroke`, `--marker-ticket-ink`. The SVGs read those directly — adding a new tone means a new `.marker-ticket--<tone>` rule and nothing else.
- `--marker-ticket-fill-a → -fill-b` is the diagonal gradient (top-left → bottom-right). Olive's fill-b is a hand-picked warm peach `#EBC988` deliberately outside the olive ramp; in-family palettes (yellow / terra / mint) stay within their token family.
- Icon slot is colored via `currentColor` (consumer's `<svg fill="currentColor">` follows `--marker-ticket-ink`). Icons that need their own halo / multi-color treatment (the Biconomy cube ships with a `mix-blend-mode: plus-darker` halo baked into its SVG file) work fine too — the slot doesn't dictate fill.
...
A small monospace stamp — hairline-bordered pill or vertical stamp housing one or two monospace characters. Two variants (`default` horizontal pill, `tall` fixed-height vertical), two appearances (`light` for paper surfaces, `dark` for screenshots and dark artefacts), five tones (`neutral`, `mint`, `olive`, `yellow`, `terra`), and an `active` prop that brightens the border + ink into the tone's "selected" palette. Also accepts a `style` prop for per-instance ink overrides (e.g., the contact-form success pill's randomized color).
...
- [app/globals.css](app/globals.css) → "Page boot" — the route→appearance mapping: `--loader-screen-bg` (the full-screen field) / `--loader-lit` / `--loader-sticker` per route + the twinkle preset for the twinkle routes.

**AI notes**
- **Why it's NOT a `<Sticker>` consumer.** Two independent reasons: (1) `<Sticker>` is `'use client'`, and the patience mark must paint before hydration during the cold-cache font gate — a client subtree can't. (2) A loader is passive; the `.sticker` class ships `cursor: pointer` + hover-lift + `:active` press, which would lie on something the visitor can't touch (it's behind `pointer-events: none` and gone on gate release). So it borrows the **shadow token**, not the class. This is the deliberate boundary — don't "promote" it into the `<Sticker>` shell.
...
- **Boot exit + field.** The `.page-boot` field is tinted per route via `--loader-screen-bg` (a soft `-160` ramp step; `#000` for `/marks`). On `html.fonts-ready` the field fades via `page-boot-out` (opacity only) while the **mark** lifts off via `page-boot-mark-out` (`scale(var(--loader-exit-scale)) translateY(var(--loader-exit-lift))`). The lift transform lives on the **mark, not the field** — putting it on `.page-boot` would scale/zoom the whole saturated screen on exit. Both nulled under reduced motion.
...
The flat, chip-less "back" link — the in-flow sibling of the fixed white-on-art chip `ExitMarker`. An anchor-variant `<NavMarker role="exit">` with `arrow_back`, plus the `.return-marker` class that strips the marker shell, points the arrow LEFT, and renders the label as a dotted→solid underline link. Distinct from `NiharHomeLink` (which is a `role="project"` chip) and from `ExitMarker` (the chip exit). Colour is consumer-driven so one primitive serves multiple palettes.

**Where it lives**
- [app/components/ReturnMarker/ReturnMarker.tsx](app/components/ReturnMarker/ReturnMarker.tsx) — component, `'use client'`.
- [app/components/ReturnMarker/index.ts](app/components/ReturnMarker/index.ts) — barrel.
...
- **No enforced touch size.** The flat link sets `padding:0`; consumers that need a ≥44px mobile tap target add it themselves (e.g. /privacy bumps `.nav-marker__content` block padding in its mobile query). If a second consumer needs the same, promote a shared mobile rule then.
- **Consumers (today):** `/all` ([BenchExitMarker.tsx](app/(works)/all/components/Essay/BenchExitMarker.tsx)) — "Nihar", grey tone, links home. `/privacy` ([PrivacyReturn.tsx](app/privacy/components/PrivacyReturn.tsx)) — "Return", stationery tone, links to the route the visitor came from.
- Promoted once /privacy became the second consumer (was route-local as `.bench-exit` on /all).
...
- **Dynamic Privacy back-link:** Footer's `onClick` writes the source pathname to `sessionStorage['privacy-from']`. The privacy page's `PrivacyReturn` reads that flag in a `useLayoutEffect` to set the **destination only** — both returns (the top `ReturnMarker` "Return" and the bottom "Return to nihar.works" button) navigate back to the route you came from (`/all`, `/biconomy`, `/rr`, `/marks`, else `/`). Labels are static; only the href is dynamic. (The old toned, source-labelled `PrivacyBackLink` NavMarker was retired in the stationery redesign.)
...
- **Counters count on mount.** `Counter` counts up when the bento mounts (with the `CoverSheet` cover). The server HTML and first client render carry the **final** value (crawlers read the prerender); the drop to 0 happens pre-paint in `useLayoutEffect` — see `app/components/ANOMALIES.md` → "Counters prerender their final value". The "K" suffix counts with the number; the small "mo" unit is a separate non-animated span.
