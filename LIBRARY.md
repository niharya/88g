...
- **Mobile composition (default, `≤767px` or `max-height: 500px`):** the row recomposes from a single line into a centered column — top hairline, centered credit, mid-divider hairline, centered link cluster. The mid-divider is a second `.footer__divider--mid` element, hidden on desktop. Cells are 44 px tall (Apple HIG min tap target), horizontal padding is `var(--space-8)` so all five pipe-bracketed cells (Privacy, Resume, LinkedIn, X, GitHub) fit at 375 px without clipping. The 12 px padding used before the Resume cell joined would tip the 5-cell sum past the mat at 375 px.
...
- Mobile breakpoint matches the site standard (`max-width: 767px` OR `max-height: 500px` — the landscape-phone clause). Inside the breakpoint, the row recomposes to a centered two-row column and cell horizontal padding drops from 12 → 8 px so five cells fit at 375 px.
- The `Resume` cell is wired through a `/resume` rewrite in `next.config.mjs` rather than linking the dated PDF filename directly. Keeps the URL pretty (`nihar.works/resume`) and lets the file in `public/` be re-versioned (e.g. `nihar-bhagat-resume-2026.pdf`) without changing the LINKS entry — just update the rewrite destination.
