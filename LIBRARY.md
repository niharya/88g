...
- **Hover convention is dotted-stays.** The label underline (`__title`, `__name`, `__exit-label`) keeps its dotted `text-decoration` on hover — affordance comes from the marker shell's bg-darken, not an underline swap. `__exit-label` has no resting underline so a dotted one fades in on hover. This matches the site-wide `t-btn1` link convention. The earlier "dotted dissolves to a solid `::after` bar" treatment was removed in this pass — do not reintroduce a `::after` underline on `.nav-marker__title/name/exit-label` or on `.t-btn1`.
